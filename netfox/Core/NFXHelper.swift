...
extension URLResponse
{
    ...
    func getNFXHeaders() -> Dictionary<String, Any>
    {
        return (self as? HTTPURLResponse)?.allHeaderFields as! [String : Any]? ?? Dictionary()
    }
}
...
class NFXDebugInfo {
    ...
    class func getNFXIP(_ completion:@escaping (_ result: String) -> Void)
    {
        ...
        session.dataTask(with: req as URLRequest) { (data, response, error) in
            do {
                let rawJsonData = try JSONSerialization.jsonObject(with: data!, options: [.allowFragments]) as? Dictionary<String, Any>
